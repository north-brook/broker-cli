...
def start(
    ...
    if code != 0:
        typer.echo("Failed to start daemon. Check ~/.broker/broker.log for startup errors.", err=True)
...
def restart(
    ...
    if code != 0:
        typer.echo("Failed to restart daemon. Check ~/.broker/broker.log for details.", err=True)
        raise typer.Exit(code=1)
