...
### broker portfolio
View all positions in your account.

Flags:
- `--json` — Output as JSON
- `--broker <name>` — Target a specific broker

Example:
$ broker portfolio
Symbol  Qty   Value      P/L
AAPL    100   $19,240    +$720
NVDA    50    $47,100    +$2,100
```

When an agent reads this, it immediately knows:
- There's a `broker portfolio` command
- It accepts `--json` and `--broker` flags
- The output is a table with specific columns
- It can parse the JSON output for programmatic use
