...
# Dry-run a rebalance order
broker check --side sell --symbol AAPL --qty 10 --limit 190
```
...
The agent:
1. Reads SKILL.md
2. Runs `broker exposure --by symbol`
3. Identifies NVDA at 47.8% of NLV
4. Calculates shares to sell
5. Gets the current price with `broker quote NVDA`
6. Runs a safety preview: `broker order sell NVDA 12 --limit 936.29 --dry-run --idempotency-key rebalance-nvda-2026-02-19`
7. Submits once the dry-run passes
8. Confirms the fill
...
broker check --side buy --symbol AAPL --qty 500 --limit 185

# Set exposure limits
broker set max_position_pct 25
